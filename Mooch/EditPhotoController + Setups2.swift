//
//  EditPhotoController + Setups2.swift
//  Mooch
//
...
extension EditPhotoController2 {
    ...
    func setupView() {
        ...
        print("1 \(pickerViewRightAnchor == nil)")
    ...
    func setupNewPicker(categories: [String], mainView: UIView, picker: UIPickerView, backgroundView: UIView, select: UIButton) {
        ...
        if showSearchBar {
            ...
            if self.currentCategory?.getName() != "Master" {
                addTitle(text: self.currentCategory?.getName()!, myView: mainView)
            } else {
